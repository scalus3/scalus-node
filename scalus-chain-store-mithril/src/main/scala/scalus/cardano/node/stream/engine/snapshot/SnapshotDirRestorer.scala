...
import scalus.cardano.node.stream.engine.snapshot.ledgerstate.{LedgerStateLayout, LedgerStateRestorer, UnsupportedLedgerSnapshotFormat}
import scalus.cardano.node.stream.engine.{ChainStore, ChainStoreUtxoSet}

import java.nio.file.{Files, Path}

/** Top-level orchestrator that restores a mithril-downloaded cardano-node snapshot directory into a
  * [[ChainStore]] + [[ChainStoreUtxoSet]].
  *
  ...
  * ==Tip alignment==
  *
  * cardano-node snapshots the ledger state periodically, while the ImmutableDB grows continuously.
  * A Mithril-signed artefact therefore typically has `immutable/` chunks that extend a few hundred
  * slots past the most recent `ledger/<Y>/` checkpoint. The orchestrator sniffs `ledger/` first to
  * learn Y, then caps the ImmutableDbRestorer at slot Y so the block-history tip lines up with the
  * ledger-state tip. Blocks after Y are ignored here; they'll come from live N2N sync once the
  * provider is up.
  *
  * ==Order==
  *
  *   1. Sniff `ledger/` → expected tip slot Y.
  *   2. Block-history via [[ImmutableDbRestorer]] stopped at slot Y. The UTxO-delta machinery in
  *      `KvChainStore.appendBlock` produces a silently-wrong UTxO set here — fine, step 3
  *      overwrites it.
  *   3. Ledger state via [[LedgerStateRestorer]]. Wipes the UTxO / utxo-by-key keyspaces and
  *      bulk-replaces them with the authoritative set stored in `ledger/<Y>/tables`, pinned at the
  *      tip established by step 2.
  */
