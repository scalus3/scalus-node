...
/** Classification of the on-disk shape of a cardano-node `db/ledger/<slot>/` directory.
  * cardano-node writes one of three layouts depending on its `LedgerDbFlavorArgs` setting (see
  ...
  *   - [[LedgerStateLayout.LegacyPreUtxoHd]]: a single flat CBOR file whose name is the slot
  *     number. Predates UTxO-HD; no aggregator produces this on a post-Chang chain.
  *   - [[LedgerStateLayout.LsmTables]]: `meta` + `state` + `tables/…` (LSM-tree files, not `tvar`).
  *     Used by stake-pool-scale deployments that can't hold the UTxO set in RAM.
  */
enum LedgerStateLayout {

    /** UTxO-HD V2 InMemory snapshot — three files under a slot-numbered directory: `meta` (JSON),
      * `state` (CBOR ExtLedgerState), and `tables` (CBOR-wrapped map of MemPack-packed UTxO
      * entries). Confirmed against real preview aggregator output at
      * `/tmp/mithrill-preview/ledger/110350526/` (2026-04-24): `tables` is a plain file, not a
      * `tables/tvar` subdir as the Mithril Rust detector might suggest — the Haskell consensus
      * write path at `LedgerDB/V2/InMemory.hs:162-168` `implTakeHandleSnapshot` writes
      * `mkFsPath [snapshotName, "tables"]` as a file.
      */
    ...
    /** UTxO-HD V1 LMDB or V2 LSM-tree snapshot — same `meta` + `state` prelude as InMemoryV2, but
      * `tables` is a directory (holding LMDB or LSM-tree files) rather than a single serialised map
      * file. Unsupported (tracked as M10d); our restorer rejects these.
      */
...
object LedgerStateLayout {
    ...
    /** Filenames inside a UTxO-HD InMemory snapshot directory. `tables` is a plain file here (the
      * serialised UTxO map), matching what cardano-node's Haskell write path actually produces; an
      * LMDB / LSM snapshot uses a directory of the same name instead.
      */
    ...
    /** Scan a cardano-node `ledger/` directory and return the highest-slot entry along with its
      * classification. Returns `None` if no ledger entries are present (empty `ledger/`). Does not
      * throw on unsupported variants — the caller decides whether to accept or reject them.
      */
    ...
    /** Classify a single `ledger/<x>` entry. Returns `None` for entries whose name doesn't parse as
      * a slot number (e.g. `README`, stray files), or whose shape doesn't match any of the three
      * recognised layouts.
      */
    ...
    /** Within a slot-numbered directory, decide between InMemoryV2 (`tables` is a file) and
      ...
      * `state` is treated as LSM — both get rejected at the next layer; the sniffer's job is just
      * to classify, not to validate.
      */
    ...
    /** Parse a slot number from an entry's file name. The cardano-node convention is that every
      * legitimate `ledger/<x>` entry (file or folder) is named with a base-10 slot number. Entries
      * that don't parse as `Long` are ignored.
      */
...
/** Thrown when a ledger-state directory has a layout our restorer does not support
  * ([[LedgerStateLayout.LegacyPreUtxoHd]] or [[LedgerStateLayout.LsmTables]]). The restorer fails
  * loud rather than silently degrading — the alternative ("skip the UTxO set") would hand the
  * caller a silently-wrong Heavy-mode store.
  */
