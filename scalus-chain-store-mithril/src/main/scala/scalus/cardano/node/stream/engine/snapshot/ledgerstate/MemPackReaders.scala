...
import scalus.cardano.ledger.{AddrKeyHash, AssetName, Coin, Credential, DataHash, DatumOption, MultiAsset, PolicyId, Script, ScriptHash, ScriptRef, Timelock, TransactionHash, TransactionInput, TransactionOutput, Value}
import scalus.uplc.builtin.{ByteString, Data}

import scala.collection.mutable

/** Era-specific MemPack readers for `cardano-ledger` types that appear in the UTxO set stored in a
  * UTxO-HD V2 InMemory snapshot's `tables/tvar` file.
  *
  * Each reader mirrors a concrete Haskell `instance MemPack …`. For every reader we note the source
  * module + line of the Haskell instance we port so future readers can cross-check.
  *
  ...
  * groups explicitly, which happens to be identical to "the hash bytes in their natural order" as
  * handed out by `hashToBytes` — so for our purposes the 32- and 28-byte hashes on disk are just
  * raw bytes with no length prefix.
  *
  * `Addr28Extra` and `DataHash32` (used in the `AlonzoTxOut` stake-compressed variants) are NOT
  * `PackedBytes`; they're ad-hoc tuples of 4 Word64s packed with the default instance, i.e.
...
object MemPackReaders {
    ...
    /** Fixed-size 28-byte hash (e.g. `AddrKeyHash`, `ScriptHash`, `PoolKeyHash`). Same reasoning as
      * `readHash32`; `PackedBytes28` = 3×Word64BE + 1×Word32BE = 28 raw bytes in order.
      */
    ...
    /** Read the raw bytes of a `CompactAddr`.
      ...
      * newtype, so its wire format is inherited from `MemPack ShortByteString`: a `Length` (VarLen
      * Word) followed by that many raw bytes. The raw bytes are the standard on-wire Cardano
      * address encoding (CIP-19) — first byte is the address-type header, rest is the payment +
      * optional stake credential, or a Byron encoding with CBOR attributes.
      *
    ...
    /** Read a packed-`Coin`: a bare `VarLen Word64` of lovelace.
      *
      * Reproduces `packCompactCoinM` / `unpackCompactCoinM` in the `CompactValue` MemPack instance
      * at `mary/impl/src/Cardano/Ledger/Mary/Value.hs:443, 452`. The top-level `CompactForm Coin`
      * instance is different (it is tag-prefixed for binary compatibility with
      * `CompactValueAdaOnly`), so do NOT use this function for standalone `CompactForm Coin`
      * decoding — it's only appropriate inside `CompactValue`.
      */
    ...
    /** Read a `CompactValue`:
      ...
      *   - tag 1 = multi-asset: `VarLen Word64` coin + `VarLen Word32` numMA + length-prefixed rep
      *     bytes. The rep is the flat five-region representation documented in
      *     `eras/mary/impl/src/Cardano/Ledger/Mary/Value.hs` (regions A/B/C/D/E).
      *
    ...
    /** Parse the flat `ShortByteString` "rep" of a `CompactValueMultiAsset` into a Scalus
      ...
      * `eras/mary/impl/src/Cardano/Ledger/Mary/Value.hs:485-527` — regions A (8 × n-byte Word64LE
      * amounts), B (2 × n-byte Word16LE policy-id offsets), C (2 × n-byte Word16LE asset-name
      * offsets), D (concatenated 28-byte policy ids), E (concatenated asset names).
      *
      * Asset-name length is the delta to the next distinct C offset, or `rep.length - offset` if
      * it's the last distinct offset. Duplicate offsets (same asset name used under more than one
      * policy) share a length.
      */
    private def decodeMultiAssetRep(
        ...
        while j < distinctSorted.length do {
            ...
            val nextOff =
                if j + 1 < distinctSorted.length then distinctSorted(j + 1) else rep.length
    ...
    /** Read a MemPack `Credential kr`: 1 tag byte (0 = ScriptHash, 1 = KeyHash) followed by 28 raw
      * hash bytes.
      *
    ...
    /** Read an `Addr28Extra`: 32 bytes as 4 little-endian Word64s.
      *
      * Returns the reconstructed 28-byte payment-key hash plus a two-bit "payment bits" value: bit
      * 0 = `isKeyHash` (0 = ScriptHash, 1 = KeyHash), bit 1 = `isMainnet` (0 = Testnet, 1 =
      ...
      * `big-endian(a) ++ big-endian(b) ++ big-endian(c) ++ big-endian(Word32((d shiftR 32)))`. Bits
      * 0/1 of `d` are the payment-cred / network selector.
      */
    ...
    /** Read `n` little-endian Word64s and return the 8n bytes as big-endian groups — i.e. the
      * per-word byte order is reversed. Used to reconstruct cardano-ledger hashes that pack their
      * blake2b bytes into Word64 tuples via the default (little-endian) `MemPack` but decode via
      * `PackedBytes` (big-endian). See `readAddr28ExtraAsHashAndBits` and
    ...
    /** Read a `CompactForm Coin`: tag 0 + `VarLen Word64`. This is the tag-prefixed version used
      * standalone (e.g. in TxOut tags 2 / 3); DO NOT use inside `CompactValue`, which uses the
      * tag-less [[readPackedCoinInsideCompactValue]].
      *
    ...
    /** Read a Babbage-/Conway-/Dijkstra-era `BabbageTxOut` (all three eras share the same MemPack
      * layout — Conway and Dijkstra reuse `BabbageTxOut` as their `TxOut`).
      *
    ...
    /** Thrown when a TxOut MemPack variant or sub-structure we don't yet handle is hit — keeps
      ...
      */
    final class UnsupportedTxOutVariant(msg: String, pos: Int) extends MemPack.DecodeError(msg, pos)
    ...
    /** Read a `Timelock era` MemPack value. The MemPack instance wraps a `MemoBytes`, so the
      * on-disk encoding is `packMemoBytesM . mbBytes`, i.e. the ShortByteString MemPack — `Length`
      * (VarLen) + raw bytes. The raw bytes are the original CBOR encoding of the timelock, which we
      * hand to Scalus's Borer `Decoder[Timelock]`.
      *
    ...
    /** Read a `PlutusScript era` MemPack value using **Conway-/Dijkstra-era tag conventions**
      * (V1=0, V2=1, V3=2, V4=3).
      *
      * Babbage-era snapshots only carry tags 0/1. Conway snapshots can carry 0/1/2. Dijkstra
      * snapshots can carry 0/1/2/3. This reader is appropriate for any snapshot whose tip era is
      * Conway or later; older snapshots simply won't emit the higher tags.
      *
    ...
    /** Read a raw `BinaryData era` MemPack value (no tag byte — just a length-prefixed byte string
      * whose bytes are CBOR-encoded Plutus Data). Produces a [[DatumOption.Inline]].
      *
      * Used exclusively for the TxOut tag-4 variant (`TxOutCompactDatum`), which stores a bare
      * `BinaryData` rather than a `Datum` — tag 4 outputs always carry an inline datum.
      *
      * Source: `eras/babbage/impl/.../Babbage/TxOut.hs:138-141` (`TxOutCompactDatum … !BinaryData
      * era`) and `libs/cardano-ledger-core/.../Plutus/Data.hs:145-146` (`newtype BinaryData =
      * BinaryData ShortByteString deriving newtype MemPack`).
      */
    ...
    /** Read a `Datum era` MemPack value: tag 0 = NoDatum, tag 1 = DatumHash + 32-byte hash, tag 2 =
      * Datum + BinaryData (length-prefixed CBOR-Plutus-Data bytes).
      *
      * Returns `None` for `NoDatum`. Tag 2 parses the BinaryData's raw bytes as a Plutus `Data` via
      * Scalus's Borer decoder; bad data surfaces as [[MemPack.DecodeError]].
      *
