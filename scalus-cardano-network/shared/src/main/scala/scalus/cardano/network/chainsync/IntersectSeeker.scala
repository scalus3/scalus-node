...
/** Translates the streaming-core [[StartFrom]] subscription policy into the wire-level
  ...
  * `FindIntersect([tip])` to commit. Between the two steps a new block may land on the peer —
  * the peer's header store still has `tip` so the second intersect succeeds, and the first
  * `next()` returns the block produced in the window. No events are missed.
  *
  ...
  * decoder is needed); `At(p)` is the checkpoint-resume case (M7). In M5 the provider always
  * passes `StartFrom.Tip`; the other cases exist so M7's replay path wires onto the same code.
  */
object IntersectSeeker {

    /** Drive the appropriate FindIntersect sequence against the peer.
      *
      * On success returns the peer-confirmed [[Point]] (not the tip — the caller's chain applier
      * is about to start pulling blocks; tip observation lives in subsequent `next()` results).
      * On failure the returned future carries [[scalus.cardano.network.ChainSyncError]] (most
      * commonly `NoIntersection` for an `At(p)` that the peer doesn't know about).
      */
    def seek(
