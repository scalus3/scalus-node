...
import scalus.cardano.n2n.handshake.{HandshakeMessage, NodeToNodeVersionData, RefuseReason}
...
/** Test-only loopback peer for [[NodeToNodeClient]]. Accepts a single TCP connection on an
  ...
  * complete ouroboros responder — for realistic end-to-end, see `scalus-n2n-it` which talks to
  * yaci-devkit in M9.
  */
