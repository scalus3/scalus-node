...
/** Covers [[ProtocolScope]] — the ergonomic bundle of CancelSource + Timer used by mini-protocol
  * drivers. Tests drive the timer through [[FakeTimer]] for determinism.
  */
class ProtocolScopeSuite extends AnyFunSuite {

    test("linked() creates a child source linked to scope.token") {
        val parent = CancelSource()
        val ps = new ProtocolScope(parent, new FakeTimer())
        val child = ps.linked()
    ...
    test("linked() child cancel does not propagate up to scope") {
        val parent = CancelSource()
        val ps = new ProtocolScope(parent, new FakeTimer())
        ps.linked().cancel(new RuntimeException("per-call"))
    ...
    test("schedule fires target.cancel with the provided cause on deadline") {
        ...
        val ps = new ProtocolScope(parent, timer)
        val target = ps.linked()
        val cause = new RuntimeException("deadline")
        val _ = ps.schedule(500.millis, target, cause)
    ...
    test("schedule handle.cancel() before deadline prevents firing") {
        ...
        val ps = new ProtocolScope(parent, timer)
        val target = ps.linked()
        val handle = ps.schedule(500.millis, target, new RuntimeException("deadline"))
        handle.cancel()
    ...
    test("linkedTo factory constructs a scope linked to the supplied parent") {
        ...
        val timer = new FakeTimer()
        val ps = ProtocolScope.linkedTo(root.token, timer)
        val fired = new AtomicInteger(0)
        ps.token.onCancel(() => { val _ = fired.incrementAndGet() })
        root.cancel()
        assert(fired.get == 1)
    }
}
